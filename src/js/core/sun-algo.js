// src/js/core/sun-algo.js
// Algoritmo NOAA puro (sin estado, sin DOM). Fuente unica compartida
// entre el main thread y el Web Worker.

export function calcSunTimesPure(date, lat, lng) {
    ...
    const calcTime = (isSunrise) => {
        ...
        let L = M + 1.916 * Math.sin(M * toRad) + 0.02 * Math.sin(2 * M * toRad) + 282.634;
        if (L > 360) L -= 360;
        else if (L < 0) L += 360;
